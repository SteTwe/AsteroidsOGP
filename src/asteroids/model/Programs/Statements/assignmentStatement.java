...
public class assignmentStatement<T>  extends  Statement{
    ...
    public assignmentStatement(Expression<T> value, String name, SourceLocation location){
        super(location);
        this.name = name;
        this.value = value;
    }
    ...
    public void execute() {
        ...
        else getProgram().addVariable(new Variable<T>(name, value.evaluate()));
        //TODO
    ...
    public Optional execute(Object[] actualArgs, Set<Variable> localVariables) {
        ...
        else localVariables.add(new Variable<T>(name, value.evaluate(actualArgs, localVariables)));
        if (value instanceof callFunction && ((callFunction)value).hasActiveBreak()) setActiveBreakStatement(true);
